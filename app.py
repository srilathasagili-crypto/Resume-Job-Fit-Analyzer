...
def main():
    ...
    with st.sidebar:
        st.header("Settings")
        csv_path = st.text_input("Path to job postings CSV", value="job.csv")
    ...
    try:
        ...
        st.error(
            f"Could not find '{csv_path}'. Place your job postings CSV in the app "
            "folder (or update the path in the sidebar) and reload."
        )
        st.stop()
